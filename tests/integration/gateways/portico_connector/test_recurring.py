...
class IntegrationGatewaysPorticoConnectorACHTests(unittest.TestCase):
    ...
    config.secret_api_key = (
        "skapi_cert_MXvdAQB61V4AkyM-x3EJuY6hkEaCzaMimTWav7mVfQ"  # gitleaks:allow
    )
    ...
    def test_02a_charge_that_payment_method(self):
        ...
        charge_result = (
            found_payment_method.charge(10.99)
            .with_currency("USD")
    ...
    def test_02b_void_test_02a(self):
        refund_response = (
            Transaction.from_id(self.test_02a_trans_id, PaymentMethodType.ACH)
            .void()
            .execute()
        )
        ...
        self.assertEqual(
            ReportingService.transaction_detail(self.test_02a_trans_id)
            .execute()
            .status,
            "V",
        )
    ...
    def test_03_create_payment_schedule(self):
        found_payment_method = RecurringPaymentMethod.find(self.ACH_id)
        found_payment_method.add_schedule(self.schedule_id).with_status(
            "Active"
        ).with_amount(12.34).with_currency("USD").with_start_date(
            datetime.date(2022, 1, 1)
        ).with_frequency(
            "Weekly"
        ).with_reprocessing_count(
            2
        ).with_email_receipt(
            "All"
        ).create()
